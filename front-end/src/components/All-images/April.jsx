...
const April = () => {
  ...
  useEffect(() => {
    const fetchGalleryItems = async () => {
      try {
        const response = await axios.get('http://localhost:9000/api/images/April');
  ...
  return (
    <div className="gallery-container container py-5 mt-5">
      <h2 className="text-center mb-4 gallery-title">April Gallery</h2>
      ...
      {loading ? (
        ...
        <Alert variant="info" className="text-center">No images found for April</Alert>
      ) : (
          ...
          {galleryItems.map((item) => (
            <div key={item._id} className="col-6 col-sm-4 col-md-4 col-lg-3">
              ...
              >
                {item.url ? (
                  <img
                    src={item.url} // Using the URL for images instead of base64 data
                    alt={item.title}
        ...
        {selectedItem && (
          <>
            ...
            <Modal.Body className="text-center">
              {selectedItem.url ? (
                <img
                  src={selectedItem.url} // Using the URL for images
